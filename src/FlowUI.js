...
export function Empty({title,text,action,onPress,icon='book'}) {return <Card style={{padding:24,alignItems:'center',gap:12}}><View style={f.emptyIcon}><Icon name={icon} color={colors.purple} size={28}/></View><T bold style={{fontSize:18,textAlign:'center'}}>{title}</T><T style={[f.body,{textAlign:'center'}]}>{text}</T>{action&&<Button onPress={onPress}>{action}</Button>}</Card>;}
