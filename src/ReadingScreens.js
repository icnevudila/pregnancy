    ...
    {a.sections.map((section,i)=><View key={section.title} style={{gap:9,paddingVertical:10}}>
      ...
      {section.image && (
        <View style={{marginVertical:8,height:210,borderRadius:18,overflow:'hidden',backgroundColor:'#F0EAF1',position:'relative'}}>
          <Image source={generatedAssets[section.image] || getAsset(section.image)} style={StyleSheet.absoluteFill} resizeMode="contain"/>
        </View>
      )}
      {section.caption && (
...
const r=StyleSheet.create({articleRow:{flexDirection:'row',alignItems:'center',gap:12,paddingVertical:13,borderBottomWidth:1,borderColor:colors.line},thumb:{height:85,width:85,borderRadius:15},cover:{height:245,borderRadius:23,overflow:'hidden',marginTop:6,backgroundColor:'#F6F0F3'},coverCopy:{position:'absolute',bottom:22,left:22,right:18,gap:12},topic:{width:143,paddingBottom:8},topicImage:{width:143,height:103,borderRadius:17,backgroundColor:'#F6F0F3'},articleCover:{width:'100%',height:210,borderRadius:22,backgroundColor:'#F6F0F3'},week:{width:46,minHeight:53,borderRadius:17,alignItems:'center',justifyContent:'center',gap:3,backgroundColor:'#F0EAEF'},fruitStage:{borderRadius:24,overflow:'hidden',padding:22,alignItems:'center',gap:10},stats:{flexDirection:'row',alignSelf:'stretch',marginTop:10},stat:{flex:1,alignItems:'center',gap:5},number:{width:27,height:27,borderRadius:14,backgroundColor:colors.lavender,alignItems:'center',justifyContent:'center'},quick:{flex:1,backgroundColor:'#FFFDF9',borderWidth:1,borderColor:colors.line,borderRadius:20,padding:14,gap:8}});
